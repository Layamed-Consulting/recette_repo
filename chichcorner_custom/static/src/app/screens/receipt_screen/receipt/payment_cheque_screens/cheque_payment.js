...
export class ChequePaymentFormPopup extends AbstractAwaitablePopup {
    static template = "chichcorner_custom.ChequePaymentForm";
    ...
    validateFields() {
        const { identite_number, cheque_number, banque } = this.state;
        this.state.isFieldEmpty =
            !identite_number.trim() || !cheque_number.trim() || !banque.trim();
    }
    ...
    async confirm() {
        ...
        if (currentOrder) {
            ...
            result = { confirmed: true, payload: { cin, cn, bn } };
        }

        // Close the popup and show the success notification
        this.props.close();
        this.notification.add(
            _t("Cheque Payment created successfully"),
            { type: "success" }
        );
