...
#Api pour Cout logistique
class ApparticleAPI(http.Controller):
    @http.route("/api/articles", auth='none', type='http', methods=['GET'], csrf=False)
    def get_articles(self, **kwargs):
        try:
            ...
            articles = request.env['x_app_facture'].sudo().search([])

            data = []
            for article in articles:
                data.append({
                    "Description": article.x_name,
                    "Marque": article.x_studio_marque,
                    "Date": article.x_studio_date_cration,
                })

            return request.make_json_response(data, status=200)

        except Exception as e:
            error_message = f"Error fetching articles: {str(e)}"
            request.env.cr.rollback()
            return http.Response(
                json.dumps({"error": "Internal Server Error", "details": error_message}),
                status=500,
...
class MultiDBAPI(http.Controller):
    ...
    @http.route('/api/get_databases', auth="none", type='http', methods=['GET'], csrf=False)
    ...
    @http.route('/api/get_products', auth="none", type='http', methods=['GET'], csrf=False)
        ...
        try:
            ...
            for line in inventory_lines:
                ...
                inventory_data.append({
                    ...
                    "ID du Produit": product.id,
                    #"Prix de vente": product.pricelist_price,
                    "Nom du Produit": product.display_name if product else "",
                    "Magasin": ", ".join(pos_categories) if pos_categories else "Aucune",
                    "Catégorie": " / ".join(category_path) if category_path else "Non classé",
                    "Quantité en stock": line.inventory_quantity_auto_apply,
                    "Coût de revient (HT)": product.x_studio_cvunits_ht_1 or 0.0,
                    "Coût de revient en stock": product.x_studio_cout_de_revient_stock_1 or 0.0,
                    ...
                    for quant in quant_records:
                        if quant.location_id.warehouse_id:
                            warehouse_info.append({
                                "Warehouse ID": quant.location_id.warehouse_id.id,
                                "Warehouse Name": quant.location_id.warehouse_id.name,
